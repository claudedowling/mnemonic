...
function validateMergePlan(
  ...
  if (sourceIds.length < 2) {
    return {
      ok: false,
      message: "execute-merge requires at least two distinct sourceIds.",
      warning: "execute-merge requires at least two distinct sourceIds.",
    };
  }
  ...
  if (!targetTitle) {
    return {
      ok: false,
      message: "execute-merge requires a non-empty targetTitle.",
      warning: "execute-merge requires a non-empty targetTitle.",
    };
  }
  ...
  for (const id of sourceIds) {
    ...
    if (!entry) {
      return {
        ok: false,
        message: `Source note '${id}' not found.`,
        warning: `Source note '${id}' not found.`,
      };
    }
    if (!entry.vault.writable) {
      return {
        ok: false,
        message: attachedVaultErrorMessage(id, entry.vault),
        warning: `Source note '${id}' is in an attached vault and cannot be modified.`,
      };
    }
  }
...
function buildCombinedTags(sourceEntries: NoteEntry[], explicitTags?: string[]): string[] {
  return explicitTags ?? Array.from(new Set(sourceEntries.flatMap((e) => e.note.tags)));
}

function computeActionLabels(mode: ConsolidationMode): { action: string; sourceSummary: string } {
  switch (mode) {
    case "delete":
...
function buildMergeEvidence(
  ...
  return { noteEvidence, mergeWarnings, mergeRisk };
}

function formatMergeLines(
  sourceIds: string[],
  targetId: string,
  consolidationMode: ConsolidationMode,
  targetVault: Vault,
  existingTargetEntry: NoteEntry | undefined,
  persistence: PersistenceStatus,
  evidence: ConsolidateExecuteMergeEvidence | undefined,
): string[] {
  const lines: string[] = [];
  ...
  if (evidence) {
    ...
    lines.push(`  Merge risk: ${evidence.mergeRisk}`);
  }

  return lines;
}

// ── Source handling per consolidation mode ────────────────────────────────────
...
async function handleDeleteMode(
  ...
  vaultManager: ServerContext["vaultManager"],
): Promise<void> {
  for (const entry of sourceEntries) {
...
async function handleSupersedesMode(
  sourceEntries: NoteEntry[],
  targetId: string,
  now: string,
  vaultChanges: Map<Vault, string[]>,
...
async function applyConsolidationMode(
  ctx: ServerContext,
  consolidationMode: ConsolidationMode,
  sourceEntries: NoteEntry[],
  sourceIds: string[],
  ...
  switch (consolidationMode) {
    case "delete":
      await handleDeleteMode(ctx, sourceEntries, sourceIds, vaultChanges, vaultManager);
      break;
    case "supersedes":
      await handleSupersedesMode(sourceEntries, targetId, now, vaultChanges, vaultManager);
      break;
    default: {
      const _exhaustive: never = consolidationMode;
      throw new UnknownConsolidationModeError(_exhaustive);
...
// ── Per-vault commit logic ──────────────────────────────────────────────────

async function commitVaultMergeChanges(
  ctx: ServerContext,
  vaultChanges: Map<Vault, string[]>,
  targetVault: Vault,
  targetId: string,
  ...
  targetCommitBody: string | undefined;
  targetCommitMessage: string | undefined;
  targetCommitFiles: string[] | undefined;
}> {
  const { action, sourceSummary } = computeActionLabels(consolidationMode);
...
export async function executeMerge(
  ...
  const validation = validateMergePlan(sourceIds, targetTitle, entries, project);
  if (!validation.ok) {
    ...
    return { content: [{ type: "text", text: validation.message }], structuredContent };
  }

  const sourceEntries = entries.filter((e) => sourceIds.includes(e.note.id));
