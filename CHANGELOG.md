...
- Recall now scores semantic candidates from note metadata plus each note's persisted projection signals instead of reading every candidate's full body before ranking. Steady-state recalls (projections current) no longer hydrate qualifying candidates; only genuinely stale or missing projections, and the final selected results, trigger a full-body read. Graph-spread candidates are also resolved and filtered on metadata only (vault resolution and visibility/temporal checks read frontmatter, not bodies), deferring full hydration to the final selected results.
- Recall no longer scores a semantic candidate with empty signals when its projection is unavailable (note unreadable or missing). Such candidates are now skipped so they cannot displace a readable note and then disappear during rendering.
- A relationship-only update no longer causes repeated body reads on recall. When a note's content is unchanged but its timestamp advanced, the stored projection is persisted with the new timestamp, so later metadata-only recalls reuse it instead of re-reading the body. Stale cached projections are also written back to the session cache.
- `setSessionCachedEmbeddings` no longer creates a partial vault cache (empty note collections) when no complete cache exists; it is a no-op so normal cache construction builds the full snapshot. Rebuilt and merged embeddings are sorted by id so score-tie ranking stays deterministic regardless of worker completion order.
...
- `recall` results now include per-result `signalStrength` — a composite quality signal (0.00-0.50) from role, centrality, lifecycle, and recency — alongside confidence tiers now derived from this richer dimensional score rather than a single coarse heuristic.

### Changed

- `mnemonic-rpi-workflow` skill: Review phase now always requires a fresh-context subagent with adversarial posture — constraint violation hunting, deliverable completeness check, and fresh verification. Removes same-context self-review option. Review handoff template includes constraint checklist and 6-point adversarial mandate.
